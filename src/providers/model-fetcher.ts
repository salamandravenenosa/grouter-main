...
export async function fetchAndSaveProviderModels(
  ...
  // Find a credential if not provided:
  // - API-key providers: api_key
  // - OAuth providers: access_token (when /models accepts bearer OAuth tokens)
  let key = apiKey;
  let providerDataRaw: string | null = null;
  if (!key) {
    const connections = listConnectionsByProvider(providerId);
    const active = connections.find(
      (c) => c.is_active === 1 && (c.api_key || c.access_token),
    );
    key = active?.api_key ?? active?.access_token ?? undefined;
    providerDataRaw = active?.provider_data ?? null;
  }

  const modelsUrl = provider.modelsUrl ?? (
    provider.baseUrl ? `${provider.baseUrl.replace(/\/$/, "")}/models` : ""
  );
  ...
  try {
    ...
    if (providerId === "kilocode") {
      const providerData = parseProviderData(providerDataRaw);
      const orgId = typeof providerData?.orgId === "string" ? providerData.orgId : null;
