...
import { codexChunkToOpenAI, newCodexStreamState, translateCodexNonStream, translateCodexSSEToNonStream } from "./codex-translator.ts";
import { geminiChunkToOpenAI, newGeminiStreamState, translateGeminiNonStream } from "./gemini-translator.ts";
import { finalizeKiroStream, kiroChunkToOpenAI, newKiroStreamState, translateKiroNonStream } from "./kiro-translator.ts";
...
export function startServer(port: number) {
  return Bun.serve({
    hostname: process.env.GROUTER_HOST ?? "0.0.0.0",
    port,
    ...
    routes: {
      ...
      "/api/auth/redirect": {
        GET: (req: Request) => handleAuthRedirect(req),
        OPTIONS: () => new Response(null, { status: 204, headers: corsHeaders() }),
      },
      "/auth/callback": {
        GET: (req: Request) => handleAuthRedirect(req),
...
async function handleChatCompletions(req: Request, pinnedProvider?: string): Promise<Response> {
  ...
  for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
    ...
    if (stream) {
      ...
      const needsLineTranslation = fmt === "claude" || fmt === "gemini" || fmt === "codex";
      const needsBinaryTranslation = fmt === "kiro";
      ...
      const kiroState = fmt === "kiro" ? newKiroStreamState(model || provider) : null;
      let tail = "";
      let lineBuf = "";
      const { readable, writable } = new TransformStream<Uint8Array, Uint8Array>({
        transform(chunk, ctrl) {
          if (!needsLineTranslation && !needsBinaryTranslation) {
            ctrl.enqueue(chunk);
          ...
          if (needsBinaryTranslation) {
            const translated = kiroChunkToOpenAI(chunk, kiroState!);
        ...
        flush(ctrl) {
          if (kiroState) {
            for (const out of finalizeKiroStream(kiroState)) {
              ctrl.enqueue(enc.encode(out));
              tail += out;
            }
          }

          const stateUsage = claudeState?.usage ?? geminiState?.usage ?? kiroState?.usage ?? null;
          const usage = (needsLineTranslation || needsBinaryTranslation) && stateUsage
    ...
    if (dispatch.format === "kiro") {
      ...
      data = translateKiroNonStream(raw, model || provider);
    } else if (dispatch.format === "codex") {
      const rawText = await upstreamResp.text();
      if (/(^|\n)\s*data:/m.test(rawText)) {
        data = translateCodexSSEToNonStream(rawText);
      } else {
        data = translateCodexNonStream(JSON.parse(rawText) as Record<string, unknown>);
      }
    } else {
