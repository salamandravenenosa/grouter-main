...
// Sweep expired pending OAuth sessions. unref() so short-lived CLI commands
// do not stay open just because the cleanup timer exists.
...
export async function importToken(providerId: string, input: string, meta?: Record<string, unknown>): Promise<Connection> {
  const adapter = requireAdapter(providerId);
  if (!adapter.importToken) {
    throw new Error(`Provider ${providerId} does not support auth.json/token import`);
  }
