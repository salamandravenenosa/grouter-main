...
import { mkdirSync, existsSync, renameSync, accessSync, constants } from "node:fs";
...
function getDbPath(): string {
  const preferredDir = join(homedir(), ".grouter");
  migrateLegacyDir(preferredDir);
  try {
    if (!existsSync(preferredDir)) mkdirSync(preferredDir, { recursive: true });
    accessSync(preferredDir, constants.W_OK);
    return join(preferredDir, "grouter.db");
  } catch {
    // Some managed platforms mount HOME on non-writable volumes.
    // Fallback keeps the service alive (ephemeral state) instead of crashing.
    const fallbackDir = "/tmp/.grouter";
    if (!existsSync(fallbackDir)) mkdirSync(fallbackDir, { recursive: true });
    accessSync(fallbackDir, constants.W_OK);
    return join(fallbackDir, "grouter.db");
  }
}
