...
const KEEP_ALIVE_INTERVAL_MS = Number.parseInt(process.env.GROUTER_KEEP_ALIVE_MS ?? "", 10) || (4 * 60 * 1000);
const KEEP_ALIVE_PATHS = ["/health", "/api/status", "/v1/models"] as const;
let keepAliveTimer: ReturnType<typeof setInterval> | null = null;

function shouldEnableKeepAlive(): boolean {
  if (process.env.GROUTER_KEEP_ALIVE === "0" || process.env.GROUTER_KEEP_ALIVE === "false") return false;
  if (process.env.GROUTER_KEEP_ALIVE === "1" || process.env.GROUTER_KEEP_ALIVE === "true") return true;
  return process.env.RENDER === "true";
}

function resolveKeepAliveBaseUrl(port: number): string {
  const externalUrl = process.env.RENDER_EXTERNAL_URL?.trim();
  if (externalUrl) return externalUrl.replace(/\/+$/, "");
  return `http://127.0.0.1:${port}`;
}

function startKeepAlivePings(port: number): void {
  if (keepAliveTimer || !shouldEnableKeepAlive()) return;

  const baseUrl = resolveKeepAliveBaseUrl(port);
  let index = 0;

  const ping = async () => {
    const path = KEEP_ALIVE_PATHS[index % KEEP_ALIVE_PATHS.length];
    index++;
    try {
      await fetch(`${baseUrl}${path}`, {
        method: "GET",
        headers: { "User-Agent": "grouter-keepalive" },
      });
    } catch {
      // Keep-alive errors should not affect the main proxy process.
    }
  };

  keepAliveTimer = setInterval(() => { void ping(); }, KEEP_ALIVE_INTERVAL_MS);
  void ping();
  console.log(`  ${chalk.gray("keep-alive")} ${chalk.green("on")} ${chalk.gray(`${baseUrl} every ${Math.round(KEEP_ALIVE_INTERVAL_MS / 60000)}m`)}`);
}
...
export function serveOnCommand(options: { port?: number }): void {
  ...
  const port = options.port ?? resolveDefaultPort();
